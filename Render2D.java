        ...
        stage.getScene().setOnKeyPressed(event -> {
            switch (event.getCode()){
                case TAB -> {
                    ...
                    Render3D.return_to_current_map(stage, map.grid2D);
                }
                case W -> {
                ...
                case M -> {
                    if (Render3D.is_music_playing) {
                        Render3D.music.pause();
                        Render3D.is_music_playing = false;
                    }
                    else {
                        Render3D.music.play();
                        Render3D.is_music_playing = true;
                    }
                }
            }
        });
